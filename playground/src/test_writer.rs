...
use std::io::{Cursor, Write, Result as IoResult, Read};
use std::rc::Rc;
...
impl TestWriterEnvelope{
	...
	pub fn read_string(&self,buffer:&mut String)->IoResult<usize>{
		assert!(!self.state.borrow().is_writer_dropped,"Not dropped.");
...
mod tests{
	...
	fn is_dropped_test(){
		...
		{
			let writer=fixture.publish().unwrap();
		}
