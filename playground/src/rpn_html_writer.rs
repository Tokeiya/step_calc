...
fn write_expression(expression:&Expression,writer:&mut dyn Write)->AnyResult<()>{
	let a=Infix::formatter::minimal_infix_notation(expression);
	writer.write_fmt(format_args!("{a}"))?;
	
	Ok(())
}

fn write_state(recent:&str,input: &VecDeque<Rpn::Token>, stack:&Vec<Expression>,writer: &mut dyn Write) -> AnyResult<String> {
	let formula=token_to_string(&input);
	
	_=writer.write(br#"<div class="step">
<h2>"#)?;
	...
	writer.write_fmt(format_args!("<h3>Recent:{recent}</h3>\n<table>\n<tr>\n<th>TOP</th>\n</tr>\n"))?;
	
	for elem in stack.iter().rev() {
		_=writer.write(b"<tr>\n<td>\n")?;
		
		write_expression(elem,writer)?;
		
		_=writer.write(b"\n</td>\n</tr>\n")?;
	}
	
	_=writer.write(br##"<tr>
<th>Bottom</th>
</tr>
</table>
</div>
...
mod tests {
	use std::io::Cursor;
	use syntax::expression::Expression;
	use syntax::number_value::NumberValue::Integer;
